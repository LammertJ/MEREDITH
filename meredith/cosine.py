...
def cosine(user_recommendation: str, model_recommendation: str) -> str:
    ...
    emb1 = np.array(
        text_embedding_model.get_embeddings([user_recommendation])[0].values
    )
    emb2 = np.array(
        text_embedding_model.get_embeddings([model_recommendation])[0].values
    )
